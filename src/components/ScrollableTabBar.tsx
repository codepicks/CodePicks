...
class ScrollableTabBar extends Component<Props, State> {
  ...
  renderTab(
    ...
    page: string,
    isTabActive: boolean,
    onPressHandler: (page: string) => void,
    onLayoutHandler: () => void
  ) {
    const tabStyle = isTabActive ? styles.tabActive : styles.tabInActive;
    const tabTextStyle = isTabActive
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {this.props.tabs.map((name: string, page: string) => {
  ...
  measureTab(page: string, event: any) {
